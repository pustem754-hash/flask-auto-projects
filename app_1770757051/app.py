...
HTML = """<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Учёт счётчиков</title>
<style>
...
def index():
    ...
    return render_template_string(HTML, residents=residents, apartments=apartments, name="Учёт счётчиков")

@app.route("/health")
def health():
    return jsonify({"status": "ok", "app": "Учёт счётчиков"})
